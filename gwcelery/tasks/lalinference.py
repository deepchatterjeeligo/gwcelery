...
@app.task(ignore_result=True, shared=False)
def _generate_and_upload_url(rundir, pe_results_path, graceid, approximant):
    """Generate the summary pages using PESummary.

    Parameters
    ----------
    rundir: str
        the path to the lalinference run directory
    pe_results_path: str
    ...
    arguments = ["summarypages.py",
                 ...
                 "--config", path_to_ini_file,
                 "--psds"
    ]

    for psd in psd_files:
        arguments.append(psd)
    ...
    try:
        ...
        bayes_baseurl = urllib.parse.urljoin(
                            app.conf['pe_results_url'],
                            os.path.relpath(
                                path_to_posplots,
                                app.conf['pe_results_path']
        ...
        gracedb.upload.delay(
            filecontents=None, filename=None, graceid=graceid,
            message=('Failed to generate summary pages with PESummary. '
                     'Results generated by the LALInference code can be '
                     'viewed <a href={}>here</a>').format(
                         bayes_baseurl), tags='pe'
        )
...
def start_pe(ini_contents, preferred_event_id, superevent_id):
    ...
    approx = ini_contents.split("approx=")[1].split("\n")[0]
    approximant = approx if "pseud" not in approx else approx.split("pseud")[0]
    ...
    (
        ...
        |
        _generate_and_upload_url.si(rundir, pe_results_path, superevent_id, approximant)
        |
